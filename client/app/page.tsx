        ...
        <div className={`${playfair.variable} ${caveat.variable} min-h-screen bg-[#1a1614]`}>
            ...
            <div className="max-w-5xl mx-auto px-6 py-12 sm:py-16">
                ...
                <header className="text-center mb-14 fade-in">
                    ...
                    <p className="text-[0.65rem] tracking-[0.25em] uppercase text-[#a09890]/50 mb-4 font-[family-name:var(--font-playfair)]">
                        famile.xyz
                    </p>
                    <h1 className="font-[family-name:var(--font-playfair)] text-[clamp(2rem,4.5vw,3.2rem)] font-bold text-[#e8e0d8] leading-[1.1] tracking-[-0.01em]">
                        AI agents for deeper<br />family connections
                    </h1>
                    <p className="mt-4 text-[#a09890] text-sm sm:text-base max-w-lg mx-auto leading-relaxed">
                        ...
                        {agentEntries.map(([name, meta]) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {influence && (
                                        <p className="text-[0.65rem] tracking-[0.08em] uppercase text-[#706b63] mb-2">
                                    ...
                                    {take && (
                                        <p className="font-[family-name:var(--font-playfair)] text-sm italic leading-relaxed text-[#c8c0b8]">
                                            &ldquo;{take.take.length > 100 ? take.take.slice(0, 100) + "…" : take.take}&rdquo;
                                        </p>
                                    )}

                                    {!take && (
                                        <p className="text-[#706b63] text-xs italic">
                                            {loading ? "Loading…" : "No take available yet today."}
                                        </p>
                                    )}

                                    <p className="mt-3 text-[0.6rem] tracking-[0.1em] uppercase text-[#706b63] group-hover:text-[#c4542b] transition-colors">
                                        Chat &rarr;
                                    </p>
                                </div>
                            );
                ...
                {data && (
                    <section className="mb-14 fade-in fade-in-d5">
                        <div className="text-center mb-6">
                            <div className="inline-flex items-center gap-3 text-[#706b63] text-xs">
                                <span className="w-8 h-px bg-[#706b63]/20" />
                        ...
                        <div className="text-center mb-6">
                            <p className="font-[family-name:var(--font-playfair)] text-lg sm:text-xl italic leading-snug text-[#c8c0b8] max-w-2xl mx-auto">
                                &ldquo;{data.story.headline}&rdquo;
                            </p>
                            <p className="text-[0.65rem] tracking-[0.15em] uppercase text-[#706b63] mt-2">
                                {data.story.source} &middot; {formattedDate}
                            </p>
                        </div>

                        <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-xs text-[#706b63] mb-5">
                            {data.takes.map(t => (
                                <span key={t.agent} className="flex items-center gap-1.5">
                                    <span>{t.emoji}</span>
                                    <span className="text-[#a09890]">{t.agent}</span>
                                    <span className="text-[#504a42] max-w-[160px] truncate">&ldquo;{t.take.slice(0, 50)}&rdquo;</span>
                                </span>
                            ))}
                        </div>
                        ...
                        <div className="text-center">
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 text-xs tracking-[0.1em] uppercase text-[#a09890] hover:text-[#c4542b] transition-colors border-b border-transparent hover:border-[#c4542b]/30"
                            >
