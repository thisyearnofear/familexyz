...
export const FamilyConnectionRings: React.FC<FamilyConnectionRingsProps> = ({
  healthScore = 75,
  activeAgents = ['Wisdom', 'Intimacy', 'GenerationalBridge', 'Presence', 'Growth'],
  className = ''
}) => {
  ...
  const rings = activeAgents.map((agent, ringIndex) => {
    const agentTheme = familyTheme.agents[agent as keyof typeof familyTheme.agents];
