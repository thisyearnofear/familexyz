...
import { Heart, Zap, Target, MessageCircle, TrendingUp } from "lucide-react";
      ...
      <CardContent className="p-5 relative z-10">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className={`
              text-3xl transform group-hover:scale-125 group-hover:rotate-6 
            ...
            <div>
              ...
              <p className={`
                text-sm ${isSelected ? 'text-white/90' : 'text-gray-600'}
              `}>
                {description}
              </p>
            </div>
          ...
          {isSelected && (
            <div className={`
              ...
              bg-white text-${colorClasses.bg.replace('bg-', '')}-600 text-lg font-bold
        ...
        <div className="mb-4">
          <p className={`
            text-sm leading-relaxed ${isSelected ? 'text-white/90' : 'text-gray-700'}
          `}>
            {purpose}
          </p>
        </div>
          ...
          {benefits.length > 3 && (
            <Badge 
              ...
              className={`
                ...
                ${isSelected ? 'text-white/80 border-white/30' : 'text-gray-600'}
              `}
            >
...
export const FamilyAgentCard: React.FC<Omit<AgentCardProps, 'icon'> & { type?: string }> = ({
  type = "default",
  ...props
}) => {
  const icon = FamilyAgentIcons[type as keyof typeof FamilyAgentIcons] || FamilyAgentIcons.Default;
