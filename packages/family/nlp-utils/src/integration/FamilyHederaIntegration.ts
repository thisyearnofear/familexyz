...
export class FamilyHederaIntegration {
  ...
  private conversationCache: Map<string, FamilyConversationContext> = new Map();
  ...
  constructor(
    ...
    const enhancedMetricsConfig = {
      ...
      rewardRates: this.applyRewardMultiplier(metricsConfig?.rewardRates || {}),
    };
  ...
  async submitFamilyMilestone(
    ...
    try {
      ...
      const response = await this.hederaService.consensus.submitMessage(
        topicId,
      ...
      if (
        ...
        await this.distributeRewards(rewardAmount, participants, familyId);
      }
  ...
  private async distributeRewards(
    ...
    try {
      ...
      const immediateAmount = Math.floor(
        (amount * this.tokenomics.distributionRules.immediateReward) / 100,
      );
      const savingsAmount = Math.floor(
        (amount * this.tokenomics.distributionRules.savingsReward) / 100,
      );
      ...
      participants.forEach((participantId) => {
        transferTx
          .addTokenTransfer(tokenId, treasuryId, -rewardPerParticipant)
          .addTokenTransfer(
            tokenId,
  ...
  private applyRewardMultiplier(
    ...
    Object.keys(rates).forEach((key) => {
      rates[key as FamilyInteractionType] = Math.floor(
        rates[key as FamilyInteractionType] * this.config.rewardMultiplier,
      );
    });
  ...
  private calculateOverallHealthScore(metrics: Record<string, number>): number {
  ...
  private async identifyCurrentChallenges(familyId: string): Promise<string[]> {
