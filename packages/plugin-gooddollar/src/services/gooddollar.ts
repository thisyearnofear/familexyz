...
export class GoodDollarService extends Service {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        try {
            ...
            if (privateKey) {
                ...
                elizaLogger.info(
                    `🎯 GoodDollar wallet initialized: ${this.wallet.address}`,
                );
            }
            ...
            elizaLogger.success(
                `✅ GoodDollar service initialized on ${this.config.network}`,
            );
        } catch (error) {
            elizaLogger.error(
                "❌ Failed to initialize GoodDollar service:",
                error,
            );
            throw error;
        }
    ...
    private async verifyConnection(): Promise<void> {
        try {
            ...
            elizaLogger.info(
                `🪙 Connected to ${tokenName} (${tokenSymbol}) with ${decimals} decimals`,
            );

            if (this.wallet) {
                const balance = await this.getBalance(this.wallet.address);
                elizaLogger.info(
                    `💰 Wallet balance: ${ethers.formatEther(balance)} G$`,
                );
            }
        } catch (error) {
            throw new Error(
                `Failed to verify GoodDollar connection: ${error.message}`,
            );
        }
    }
    ...
    async transfer(
        ...
        amount: string,
        data?: string,
    ): Promise<GDollarTransaction> {
        if (!this.wallet) {
            throw new Error(
                "Wallet not initialized. Cannot perform transfers.",
            );
        }
        ...
        try {
            ...
            if (data) {
                ...
                tx = await this.tokenContract!.transferAndCall(
                    to,
                    amountWei,
                    data,
                );
                elizaLogger.info(
                    `📤 transferAndCall sent: ${amount} G$ to ${to} with data`,
                );
            } else {
                // Standard transfer
    ...
    async claimUBI(): Promise<UBIClaimResult> {
        ...
        try {
            ...
            const canClaim = await this.tokenContract!.canClaim(
                this.wallet.address,
            );
            if (!canClaim) {
                const lastClaimedTimestamp =
                    await this.tokenContract!.lastClaimed(this.wallet.address);
                throw new Error(
                    `Cannot claim UBI yet. Last claimed: ${new Date(Number(lastClaimedTimestamp) * 1000)}`,
                );
            }
            ...
            elizaLogger.success(
                `✅ UBI claimed: ${ethers.formatEther(claimAmount)} G$`,
            );
    ...
    // Add a public method to check UBI claim eligibility
    async canClaimUBI(address: string): Promise<boolean> {
        if (!this.tokenContract) {
            return false;
        }

        try {
            return await this.tokenContract.canClaim(address);
        } catch (error) {
            elizaLogger.error("Failed to check UBI claim eligibility:", error);
            return false;
        }
    }

    // Add missing method for API compatibility
    async getSuperTokenBalance(address: string): Promise<string> {
        // For now, return 0 as we don't have SuperToken integration fully implemented
        return "0";
    }

    async distributeFamilyReward(
        reward: GDollarReward,
    ): Promise<GDollarTransaction> {
        if (!this.wallet) {
            throw new Error(
                "Wallet not initialized. Cannot distribute rewards.",
            );
        }
        ...
        try {
            ...
            elizaLogger.success(
                `🎁 Family reward distributed: ${ethers.formatEther(reward.amount)} G$ for ${reward.reason}`,
            );
    ...
    async getTransactionHistory(
        ...
        try {
            ...
            const cachedHistory =
                this.cache.get<GDollarTransaction[]>(cacheKey);
            if (cachedHistory) {
            ...
            const transferAndCallFilter =
                this.tokenContract!.filters.TransferAndCall(null, address);
            const transferAndCallEvents = await this.tokenContract!.queryFilter(
                transferAndCallFilter,
                -1000,
            );
            ...
            for (const event of allEvents.slice(-limit)) {
                ...
                const transaction: GDollarTransaction = {
                    ...
                    type:
                        parsedEvent!.name === "TransferAndCall"
                            ? "transferAndCall"
                            : "transfer",
                    data:
                        parsedEvent!.name === "TransferAndCall"
    ...
    async estimateGas(
        ...
        try {
            ...
            if (data) {
                gasEstimate =
                    await this.tokenContract!.transferAndCall.estimateGas(
                        to,
                        amountWei,
                        data,
                    );
            } else {
                gasEstimate = await this.tokenContract!.transfer.estimateGas(
                    to,
                    amountWei,
                );
            }
